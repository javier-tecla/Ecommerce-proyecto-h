...
@section('content')
    <h1>Modificación de datos del usuario: {{ $usuario->name }}</h1>
    <hr>
                                    ...
                                    <div class="input-group">
                                        ...
                                        <select name="rol" id="rol" class="form-control" required>
                                            ...
                                            @foreach ($roles as $rol)
                                                ...
                                                <option value="{{ $rol->name }}" {{ old('rol',$usuario->roles->pluck('name')->implode(', ')) == $rol->name ? 'selected' : '' }}>{{ $rol->name }}</option>
                                                @endif
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="name" id="name" class="form-control"
                                            value="{{ old('name',$usuario->name) }}" placeholder="Nombre completo" required>
                                    </div>
                                    @error('name')
                                        <small class="text-danger">{{ $message }}</small>
                                    @enderror
